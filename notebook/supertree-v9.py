...
SVG_OUT = r"vis\ctann-v9.svg"
PNG_OUT =  r"vis\ctann-v9.png"

DEFAULT_NODE_FILL_COLOR = "#7A7B78"
MARTIN_NODE_FILL_COLOR = "#1E90FF"
CHENCHEN_NODE_FILL_COLOR = "#E31A1C"
...
def main() -> None:
    ...
    for node, (x, y) in positions.items():
        ...
        terminal_sources = leaf_sources.get(node, set())
        is_martin_leaf = "martin" in terminal_sources
        is_chenchen_leaf = "chenchen" in terminal_sources

        if is_martin_leaf and is_chenchen_leaf:
            # Retain both source categories for terminal nodes shared by the two data sets.
            ax.add_patch(Wedge(center, node_d / 2, 90, 270, linewidth=0, facecolor=MARTIN_NODE_FILL_COLOR, zorder=2))
            ax.add_patch(Wedge(center, node_d / 2, 270, 450, linewidth=0, facecolor=CHENCHEN_NODE_FILL_COLOR, zorder=2))
        else:
            fill_color = (
                MARTIN_NODE_FILL_COLOR if is_martin_leaf
                else CHENCHEN_NODE_FILL_COLOR if is_chenchen_leaf
                else DEFAULT_NODE_FILL_COLOR
            )
            ax.add_patch(
                Circle(
                    center,
    ...
    martin_leaf_count = sum(1 for sources in leaf_sources.values() if "martin" in sources and "chenchen" not in sources)
    chenchen_leaf_count = sum(1 for sources in leaf_sources.values() if "chenchen" in sources and "martin" not in sources)
    shared_leaf_count = sum(1 for sources in leaf_sources.values() if "martin" in sources and "chenchen" in sources)
    other_node_count = len(node_sources) - martin_leaf_count - chenchen_leaf_count - shared_leaf_count

    legend_items = [
        (
            DEFAULT_NODE_FILL_COLOR,
            f"All other nodes ({other_node_count} nodes)",
        ),
        (
            MARTIN_NODE_FILL_COLOR,
            f'Martin terminal leaves ({martin_leaf_count} nodes)',
        ),
        (
            CHENCHEN_NODE_FILL_COLOR,
            f"Chenchen terminal leaves ({chenchen_leaf_count} nodes)",
        ),
    ]
    ...
    if shared_leaf_count:
        ax.add_patch(Wedge((legend_x + 8, item_y - 5), 7, 90, 270, linewidth=0, facecolor=MARTIN_NODE_FILL_COLOR))
        ax.add_patch(Wedge((legend_x + 8, item_y - 5), 7, 270, 450, linewidth=0, facecolor=CHENCHEN_NODE_FILL_COLOR))
        ax.text(
            legend_x + 22,
            item_y - 1,
            f"Martin + Chenchen terminal leaves ({shared_leaf_count} nodes)",
            fontsize=19,
            ha="left",
            va="top",
            color="#222222",
